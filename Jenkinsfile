  ...
  agent {label 'yi-tensorflow'}
    stages {
        stage('Build NVIDIA Basic Docker Image') {
            steps {
	       sh 'docker build --no-cache -f Dockerfile -t nvidia/cuda:10.1-cudnn7-base .'  
            }
        }
	stage('Testing Docker Image') { 
            steps {
                sh '''#!/bin/bash -xe
		    echo 'Hello, YI-TFLOW!!'
                    image_id="$(docker images -q nvidia/cuda:10.1-cudnn7-base)"
                      if [[ "$(docker images -q nvidia/cuda:10.1-cudnn7-base 2> /dev/null)" == "$image_id" ]]; then
            ...
            steps {
		     ...
		     echo 'Remove Original Docker Image' 
	             ...
	             docker rmi -f $CURRENT_ID
                     ...
                     pv -f $WORKSPACE/nvidia-cuda-10.1-cudnn7-base.tar | docker load
	             docker tag $CURRENT_ID nvidia/cuda:10.1-cudnn7-base
                        
                     echo 'Removing temp archive.'  
                     rm $WORKSPACE/nvidia-cuda-10.1-cudnn7-base.tar
                   ''' 
		    }
		}
 }
